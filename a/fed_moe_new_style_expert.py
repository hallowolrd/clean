...
Preserved Fed-MoE idea:
    ...
    x local client-model true-label confidence P_y
    -> client-level relevance -> row softmax -> moving expert fusion.

Controlled adaptations:
    * each participating client contributes one client-level relevance score;
    * each global expert aggregates only the same expert slot from clients;
    * the existing shared SparseMoEClassifier is kept unchanged;
    * no main expert is added;
    * the server router is NOT additionally trained on evidence data;
...
class FedMoEStyleExpertAggregator(Aggregator):
    """Client-level reserved-response relevance weighted expert aggregation."""
    ...
    def aggregate(
        ...
        reserved_loader, device, reserved_summary, model_builder = (
            _get_method_context_resources(self)
        )
        moving_rate = float(
            _cfg_get(
                self.cfg,
                "fed_moe.moving_rate",
                ADAPTATION_DEFAULT_MOVING_RATE,
            )
        )
        ...
        # Fed-MoE paper-style client-level relevance:
        # one source column per participating client, not per local expert.
        ...
        expected_shape = (len(expert_ids), len(client_ids))
        ...
        client_weight_matrix = F.softmax(relevance.float(), dim=1)
        if not torch.isfinite(client_weight_matrix).all():
            raise ValueError("Fed-MoE client weight matrix 出现 NaN/Inf。")
        row_sums = client_weight_matrix.sum(dim=1)
        if not torch.allclose(
            ...
            raise RuntimeError("Fed-MoE client weight matrix 的行权重和不为 1。")
        ...
        client_weights = _collapse_target_weights_to_clients(
            client_weight_matrix=client_weight_matrix,
            client_ids=client_ids,
        )
        ...
        if bool(_cfg_get(self.cfg, "fed_moe.log_detail", True)):
            print(
                ...
                f"mean_max_client_weight={diagnostics['mean_max_client_weight']:.6f} "
                f"top_clients={diagnostics['top_clients_by_global_expert']}",
                flush=True,
            )
...
def _compute_reserved_relevance(
    ...
    """Compute paper-style E x C relevance from full client-model responses.

    For each evidence sample (x, y):
        Q_e(x) comes from the current server router;
        P_y[k] comes from the complete local SparseMoE model of client k.

    The result therefore has one column per participating client, matching the
    client-level relevance structure of Fed-MoE instead of introducing an
    extra client-expert-level scoring mechanism.
    """
    num_experts = len(expert_ids)
    if num_experts <= 0:
        raise ValueError("num_experts 必须大于 0。")

    # Q uses the already-aggregated non-expert backbone/router.
    ...
    with torch.inference_mode():
        for batch in reserved_loader:
            ...
            if router_probs.size(1) != num_experts:
                raise RuntimeError(
                    "server router expert 数量不一致："
                    f"expected={num_experts}, actual={router_probs.size(1)}"
                )
    ...
    relevance_columns: List[torch.Tensor] = []
    ...
    for update in client_updates:
        ...
        column = torch.zeros(num_experts, dtype=torch.float64)
        seen = 0
            ...
            for batch_id, batch in enumerate(reserved_loader):
                ...
                if not torch.equal(targets.detach().cpu(), expected_targets):
                    raise RuntimeError(
                        ...
                        "Fed-MoE 需要 shuffle=False 的 deterministic evidence loader。"
                    )

                # P_y[k] is computed from the COMPLETE local client model,
                # including its local backbone, adapter, router and sparse MoE.
                outputs = local_model(images)
                logits = base.extract_logits(outputs)
                true_prob = F.softmax(logits.float(), dim=-1).gather(
                    dim=1,
                    index=targets.reshape(-1, 1),
                ).squeeze(1)

                q = q_batches[batch_id].to(
                    device=device,
                    dtype=torch.float32,
                )
                if q.size(0) != true_prob.size(0):
                    raise RuntimeError(
                        "server gate 与 local client confidence batch size 不一致："
                        f"Q={tuple(q.shape)}, P_y={tuple(true_prob.shape)}"
                    )

                column += q.transpose(0, 1).double().matmul(
                    true_prob.double()
                ).cpu()
        ...
        relevance_columns.append((column / float(sample_count)).float())

    relevance = torch.stack(relevance_columns, dim=1)  # [E, C]
...
def _apply_moving_expert_fusion(
    ...
    client_weight_matrix: torch.Tensor,
    moving_rate: float,
    strict: bool,
) -> None:
    """Fuse the SAME expert slot across clients using target-specific weights.

    Global expert e only receives local expert e from every participating
    client. No cross-slot source selection is performed.
    """
    update_by_client = {
    ...
    if tuple(client_weight_matrix.shape) != (len(expert_ids), len(client_ids)):
        raise ValueError(
            "client_weight_matrix shape 错误："
            f"expected={(len(expert_ids), len(client_ids))}, "
            f"actual={tuple(client_weight_matrix.shape)}"
        )
    ...
    for target_name in target_param_names:
        ...
        row = client_weight_matrix[expert_row[int(target_expert_id)]]
        fused = torch.zeros_like(target_global)

        for client_index, client_id in enumerate(client_ids):
            weight = float(row[client_index].item())
            update = update_by_client[int(client_id)]
            delta = update.model_delta.get(target_name)

            if delta is None:
                if strict:
                    raise KeyError(
                        f"客户端 {client_id} model_delta 缺少同 slot expert 参数："
                        f"{target_name}"
                    )
                source_local = target_global
            else:
                source_local = target_global + delta.to(
                    device=target_global.device,
                    dtype=target_global.dtype,
                )

            fused = fused + weight * source_local
...
def _collapse_target_weights_to_clients(
    client_weight_matrix: torch.Tensor,
    client_ids: Sequence[int],
) -> Dict[int, float]:
    """Diagnostic client weight averaged across target global experts."""
    mean_client_weight = client_weight_matrix.mean(dim=0)
    result = {
        int(client_id): float(mean_client_weight[index].item())
...
def _build_diagnostics(
    ...
    entropy = -torch.sum(
        client_weight_matrix
        * torch.log(client_weight_matrix.clamp_min(1.0e-12)),
        dim=1,
    )
    max_weights = client_weight_matrix.max(dim=1).values

    top_clients: Dict[int, List[Dict[str, Any]]] = {}
    topn = min(5, client_weight_matrix.size(1))
    for row_id, expert_id in enumerate(expert_ids):
        values, indices = torch.topk(client_weight_matrix[row_id], k=topn)
        rows: List[Dict[str, Any]] = []
        for value, index in zip(values.tolist(), indices.tolist()):
            client_id = int(client_ids[int(index)])
            rows.append(
                {
                    "client_id": client_id,
                    "weight": float(value),
        ...
        top_clients[int(expert_id)] = rows
    ...
    return {
        ...
        "num_source_clients": int(len(client_ids)),
        "relevance_shape": [int(relevance.size(0)), int(relevance.size(1))],
        "relevance_granularity": "client_model",
        "parameter_fusion_scope": "same_expert_slot",
        "moving_rate": float(moving_rate),
        "moving_rate_source": (
            "adaptation_hyperparameter_not_reported_in_public_paper"
        ),
        "reserved_size": int(reserved_summary.get("size", 0)),
        "reserved_class_counts": dict(
            reserved_summary.get("class_counts", {})
        ),
        "reserved_class_balanced": bool(
            reserved_summary.get("class_balanced", False)
        ),
        ...
        "mean_max_client_weight": float(max_weights.mean().item()),
        "min_max_client_weight": float(max_weights.min().item()),
        "max_max_client_weight": float(max_weights.max().item()),
        "top_clients_by_global_expert": top_clients,
    }
